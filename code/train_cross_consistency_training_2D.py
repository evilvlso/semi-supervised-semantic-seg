...
from dataloaders.dataset import BaseDataSets, RandomGenerator, TwoStreamBatchSampler,OwnBaseDataSets
from utils import losses, metrics, ramps
from val_2D import test_single_volume_ds,test_throat_single_volume_ds
...
parser.add_argument('--exp', type=str,
                    default='throat/Cross_Consistency_Training', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    db_train = OwnBaseDataSets(base_dir=args.root_path, split="train", num=None,output_size=args.patch_size)
    ...
    unlabeled_idxs = list(range(args.labeled_num, total_slices))
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 20 == 0:
                image = volume_batch[args.labeled_bs+1, :, :, :]
                writer.add_image('train/Image', image, iter_num)
                outputs = torch.argmax(torch.softmax(
                    outputs, dim=1), dim=1, keepdim=True)
                writer.add_image('train/Prediction',
                                 outputs[args.labeled_bs+1, ...] * 50, iter_num)
                labs = label_batch[args.labeled_bs+1, ...].unsqueeze(0) * 50
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_throat_single_volume_ds(
